...
class Lot(models.Model):
    ...
    def available_quantity(self):
        used_quantity = sum(
            movement.quantity for movement in self.movements.filter(movement_type="OUT")
        )
...
class Movement(models.Model):
    ...
    def clean(self):
        ...
        if self.movement_type == "OUT":
            past_lot_movements = Movement.objects.filter(
                lot=self.lot, movement_type="OUT"
            )
            total_num_past_out = sum(
                movement.quantity for movement in past_lot_movements
            )
            if total_num_past_out + self.quantity > self.lot.initial_quantity:
                raise ValidationError(
                    f"Insufficient quantity in lot {self.lot.lot_number}"
                )
